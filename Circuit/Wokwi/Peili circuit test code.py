...
# wokwi link https://wokwi.com/projects/396697502778161153
...
# creating buzzer, leds, servo, switch and button
buzzer = PWM(Pin(1))
#speaker = Speaker(1)
ledred = Pin(2, Pin.OUT)
ledgreen = Pin(14, Pin.OUT)
servo = Servo(15)
switch = Pin(28, Pin.IN, Pin.PULL_UP)
...
while True:
    if button.value() == 0:
        ...
        #speaker.on()
        #Starting sequence: Leds blink and buzzer beeps. Red 4 times with short beep, green 1 time with higher freq long beep
        #end = time.ticks_ms()
        #print("end tick: ")
        #print(end)
        #length = time.ticks_diff(end, start)
        #print(length)
